...
self.addEventListener('fetch', e => {
  ...
  // Only ever handle http(s). Under Electron the app runs from file://, where
  // fetch() cannot service a request — intercepting there returns a non-Response
  // and kills the navigation outright ("Failed to convert value to 'Response'").
  // Leaving these alone lets the browser load them normally.
  let url;
  try { url = new URL(req.url); } catch (_) { return; }
  if (url.protocol !== 'http:' && url.protocol !== 'https:') return;

  // Live streams and the radio API are never cached — huge, and would evict the shell.
    ...
    e.respondWith(
      fetch(req)
        ...
        .catch(() => caches.match(req)
          ...
          // respondWith REQUIRES a Response — returning undefined is a hard failure
  ...
  e.respondWith(
    caches.match(req).then(hit => {
      ...
      return fetch(req).then(r => {
        ...
        return r;
      }).catch(() => Response.error());   // never resolve to undefined
    })
  );
});
